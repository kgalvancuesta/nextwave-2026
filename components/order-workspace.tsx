...
function OfferModal({ market, onClose, onSaved }: { market: MarketState; onClose: () => void; onSaved: () => Promise<void> }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [draft, setDraft] = useState({ carrierId: market.carriers[0]?.carrier.id || "", price: "", pickupTime: "", expectedArrival: "", conditions: "", requirementsConfirmed: market.market.mandate.conditions.length === 0, isFinalOffer: false, requiresImmediateDecision: false, callbackAllowed: true });
