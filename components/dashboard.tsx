...
export function Dashboard() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-[1480px] px-4 py-5 sm:px-7 lg:px-10">
      <header className="mb-7 border-b border-[var(--ink)] pb-5">
        <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow mb-2">Order + market state</p><h1 className="text-4xl font-semibold tracking-[-0.06em] sm:text-5xl">MARKETLINE</h1><p className="mt-2 max-w-xl text-sm text-[var(--muted)]">Ground transport procurement, calls, offers, and commitments in one authoritative operating state.</p></div><div className="flex items-center gap-2 rounded-full border border-[var(--line)] bg-white px-3 py-2 text-xs font-medium"><span className={`h-2 w-2 rounded-full ${connected ? "bg-emerald-500" : "bg-[var(--danger)]"}`} />{connected ? "Live state connected" : "API unavailable"}</div></div>
        <nav aria-label="Primary" className="mt-5 flex gap-1" role="tablist"><button role="tab" aria-selected={view === "orders"} onClick={() => navigate("orders")} className={`rounded-t-xl px-5 py-2.5 font-mono text-xs uppercase tracking-wider ${view === "orders" ? "bg-[var(--ink)] text-white" : "bg-white text-[var(--muted)] hover:bg-[var(--paper)]"}`}>Orders</button><button role="tab" aria-selected={view === "phone"} onClick={() => navigate("phone")} className={`rounded-t-xl px-5 py-2.5 font-mono text-xs uppercase tracking-wider ${view === "phone" ? "bg-[var(--ink)] text-white" : "bg-white text-[var(--muted)] hover:bg-[var(--paper)]"}`}>Phone</button></nav>
      </header>
      ...
      <section>
        <div className="mb-4 flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Operational workspaces</p><h2 className="mt-1 text-2xl font-semibold tracking-tight">Orders</h2></div><div className="flex flex-wrap items-center gap-2"><div className="flex flex-wrap gap-1 rounded-xl border border-[var(--line)] bg-white p-1">{(["ALL", "ACTIVE", "SOURCING", "COMMITTED", "EXCEPTIONS", "PAST"] as OrderFilter[]).map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-lg px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider ${filter === item ? "bg-[var(--ink)] text-white" : "text-[var(--muted)] hover:bg-[var(--paper)]"}`}>{item}</button>)}</div><button onClick={() => setOrderFormOpen(true)} className="primary-button"><Plus size={17} /> New order</button></div></div>
      ...
      <section><div className="mb-4"><p className="eyebrow">Carrier operations</p><h2 className="mt-1 text-2xl font-semibold tracking-tight">Phone</h2></div><div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
