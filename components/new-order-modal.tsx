    ...
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="flex max-h-[90vh] w-full max-w-4xl flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
        <form ref={formRef} noValidate onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
          <DialogHeader className="shrink-0 border-b border-[var(--line)] px-6 py-5 text-left">
            <p className="eyebrow">New procurement workspace</p>
            <DialogTitle className="text-xl font-medium tracking-tight text-[var(--ink)]">Create order</DialogTitle>
            <DialogDescription>Define the mandate once. Every market keeps its own snapshot.</DialogDescription>
          </DialogHeader>
          <div className="min-h-0 flex-1 space-y-8 overflow-y-auto p-6">
            ...
            <section>
              ...
              <Card className="mt-4 border border-[var(--line)] bg-secondary ring-0">
                <CardContent className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label={`Exchange rate: 1 ${alternateCurrency} equals`}><div className="relative"><input data-field="exchangeRate" required type="number" min="0.000001" step="0.000001" className="field-input pr-16" value={alternateExchangeRate} onChange={(e) => { setAlternateExchangeRate(e.target.value); setError(null); }} /><span className="pointer-events-none absolute inset-y-0 right-3 flex items-center font-mono text-xs text-[var(--muted-text)]">{draft.currency}</span></div><span className="mt-1 block text-xs text-[var(--muted-text)]">Snapshotted on the order and used for every backend comparison.</span></Field>
                </CardContent>
              </Card>
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div><span className="mb-1.5 block text-sm font-medium">Latest arrival</span><label className="mb-2 flex items-center gap-2 text-sm font-normal text-[var(--muted-text)]"><input type="checkbox" checked={deadlineEnabled} onChange={(e) => toggleDeadline(e.target.checked)} className="h-4 w-4 accent-[var(--brand)]" /> Required deadline</label><input aria-label="Latest arrival deadline" data-field="mustArriveBy" aria-invalid={Boolean(visibleErrors.mustArriveBy)} disabled={!deadlineEnabled} type="datetime-local" min={draft.preferredArrival || undefined} className="field-input disabled:cursor-not-allowed disabled:opacity-50" value={draft.mustArriveBy} onChange={(e) => update("mustArriveBy", e.target.value)} />{visibleErrors.mustArriveBy && <span className="mt-1.5 block text-sm font-normal text-destructive">{visibleErrors.mustArriveBy}</span>}</div>
              </div>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              <Card className="mt-5 border border-[var(--line)] bg-secondary ring-0">
                <CardContent>
                  <div className="flex items-center justify-between gap-4"><span className={priority > 50 ? "text-base font-medium text-[var(--ink)]" : "text-sm font-normal text-[var(--muted-text)]"}>Save money</span><span className="eyebrow">{priority}% price / {100 - priority}% speed</span><span className={priority < 50 ? "text-base font-medium text-[var(--ink)]" : "text-sm font-normal text-[var(--muted-text)]"}>Arrive sooner</span></div>
                  <input aria-label="Price versus speed priority" type="range" min="0" max="100" value={priority} onChange={(e) => setPriority(Number(e.target.value))} className="mt-3 w-full accent-[var(--brand)]" />
                </CardContent>
              </Card>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <section>
              <SectionTitle number="03" title="Demurrage watch" description="Optional, but required to quantify a free-time risk and let Luna resolve it by phone." />
            ...
            <section>
              ...
              <div className="mt-4 space-y-2">
                {conditions.map((condition, index) => <div key={index} className="flex gap-2"><input className="field-input" value={condition} onChange={(e) => setConditions((current) => current.map((item, itemIndex) => itemIndex === index ? e.target.value : item))} placeholder="Price must include tolls" /><Button type="button" aria-label="Remove condition" variant="ghost" size="icon" onClick={() => setConditions((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={16} /></Button></div>)}
                <Button type="button" variant="outline" onClick={() => setConditions((current) => [...current, ""])}><Plus size={15} /> Add condition</Button>
              </div>
            </section>
            ...
            <section>
              <div className="flex items-start justify-between gap-4"><SectionTitle number="05" title="Carrier selection" description="Leave empty for Luna to rank and select carriers automatically, or choose up to three overrides." /><Button type="button" variant="outline" onClick={onAddCarrier}><Plus size={15} /> New carrier</Button></div>
              <div data-field="carrierIds" tabIndex={-1} className="mt-4 grid gap-2 sm:grid-cols-2">
                {contacts.map((contact) => { const checked = selected.has(contact.id); return <label key={contact.id} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition-colors ${checked ? "border-primary bg-[var(--ice)] ring-1 ring-primary" : "border-[var(--line)] bg-white hover:border-primary/40"}`}><input type="checkbox" checked={checked} disabled={!checked && selected.size >= 3} onChange={() => toggleCarrier(contact.id)} className="h-5 w-5 accent-[var(--brand)]" /><span className="min-w-0"><span className="block truncate font-medium text-[var(--ink)]">{contact.label}</span><span className="font-mono text-xs text-[var(--muted-text)]">{contact.e164PhoneNumber}</span></span></label>; })}
                {contacts.length === 0 && <button type="button" onClick={onAddCarrier} className="col-span-full rounded-xl border border-dashed border-[var(--line)] bg-secondary p-8 text-sm text-[var(--muted-text)] hover:border-primary/50 hover:text-[var(--ink)]">Add a carrier before creating this order.</button>}
              </div>
              <p className="eyebrow mt-3">{selected.size === 0 ? `Automatic · Luna selects ${draft.desiredCarriers}` : `${selected.size} manual override${selected.size === 1 ? "" : "s"}`}</p>
            </section>
          </div>
          <DialogFooter className="shrink-0 rounded-b-none border-t border-[var(--line)] bg-transparent px-6 py-4 sm:justify-between">
            <div className="w-full">
              {(Object.keys(visibleErrors).length > 0 || error) && <div role="alert" className="mb-4 rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive"><div className="flex items-start gap-2"><AlertTriangle size={17} className="mt-0.5 shrink-0" /><div><p className="font-medium">{error || `Please fix ${Object.keys(visibleErrors).length} field${Object.keys(visibleErrors).length === 1 ? "" : "s"} before creating the order.`}</p>{!error && <ul className="mt-1 list-disc pl-5">{Object.entries(visibleErrors).map(([field, message]) => <li key={field}>{message}</li>)}</ul>}</div></div></div>}
              <div className="flex items-center justify-between gap-4"><p className="hidden text-sm text-[var(--muted-text)] sm:block">Creates Order + Market #1. Calls begin only when you start the market.</p><div className="ml-auto flex gap-3"><Button type="button" variant="outline" onClick={onClose}>Cancel</Button><Button type="submit" disabled={busy}>{busy ? "Creating…" : "Create order"}</Button></div></div>
            </div>
          </DialogFooter>
        </form>
      </DialogContent>
...
function SectionTitle({ number, title, description }: { number: string; title: string; description: string }) { return <div><p className="eyebrow">{number}</p><h3 className="mt-1 text-base font-medium text-[var(--ink)]">{title}</h3><p className="mt-1 text-sm text-[var(--muted-text)]">{description}</p></div>; }
function Field({ label, error, wide, children }: { label: string; error?: string; wide?: boolean; children: React.ReactNode }) { return <label className={wide ? "block sm:col-span-2" : "block"}><span className="mb-1.5 block text-sm font-medium text-[var(--ink)]">{label}</span>{children}{error && <span className="mt-1.5 block text-sm font-normal text-destructive">{error}</span>}</label>; }
