    ...
    if (!context.award && !context.marketClosed) {
      return {
        ...
        instructions: [
          ...
          "NEGOTIATE: the server has already compared every carrier on this market and computed one target price. Ask once, plainly, whether they can do that exact price — for example 'Is there any room to get to $X?' Never negotiate on pickup or arrival, never invent your own number, never repeat the ask if they decline, and never pressure or persuade; record whatever they say with one update and move on. Outside a NEGOTIATE instruction, never negotiate or counter on your own — the server compares locked quotes and chooses the best feasible one.",
